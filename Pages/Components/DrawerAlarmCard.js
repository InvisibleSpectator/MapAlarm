    ...
    return (
      <Card onPress={this.props.onSelect}>
        ...
        <CardItem>
          ...
          <Icon type='MaterialIcons' onPress={()=>{if(this.state.isActive&&this.state.isLocationBound)this.props.onSelect(this.state.location)}}  style={{ position: 'absolute', right: 12, color: '#32505C' }} name={this.state.isLocationBound ? 'location-on' : 'location-off'} />
        </CardItem>
        <CardItem >
          ...
          <Switch value={Boolean(this.state.isActive)} style={{ position: 'absolute', right: 0 }} onChange={() => { Database.updateAlarm({...this.state, time:this.state.time.getTime(),isActive:!this.state.isActive},this.props.onDelete) }}></Switch>
        </CardItem>
      </Card>
    );
  }
